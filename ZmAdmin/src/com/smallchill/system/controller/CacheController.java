...
public class CacheController extends BaseController {
	...
	public ApiResult getDeptSelect() {
		final String num = getParameter("num");
	...
	public ApiResult getUserSelect() {
		final String num = getParameter("num");
	...
	public ApiResult getRoleSelect() {
		final String num = getParameter("num");
	...
	@SuppressWarnings("rawtypes")
	@Json
	@RequestMapping("/getCheckRadio")
	public ApiResult getCheckRadio() {
		String type = getParameter("type");
		String source = getParameter("source");
		List<Map> list = new ArrayList<>();
		if (type.equals("checkradioDiy")) {
			String where = getParameter("where");
			Map param = CMap.createHashMap();
			if (StrKit.notBlank(where)) {
				param = JsonKit.parse(where, Map.class);
			}
			list = Md.selectListByCache(SYS_CACHE, CHECK_RADIO + "diy_" + source, source, param, Map.class);
		} else {
			String code = type.split("_")[1];
			list = Db.selectListByCache(SYS_CACHE, CHECK_RADIO + "dict_" + code, "select num as \"id\",pId as \"pid\",name as \"text\" from  BLADE_DICT where code=#{code} and num>0", CMap.init().set("code", code));
		}
		return json(list);
	}
	...
	public ApiResult getDicById() {
		final int id = getParameterToInt("id");
		List<Map<String, Object>> dict = CacheKit.get(SYS_CACHE, DICT_CODE + id,
				new ILoader() {
					public Object load() {
						return Db.selectList("select CODE as \"code\" from BLADE_DICT where id=#{id}", CMap.init().set("id", id));
					}
				});
		return json(dict);
	}
	...
	public ApiResult menuTreeListByRoleId() {
	...
	public ApiResult roleTreeListById() {
