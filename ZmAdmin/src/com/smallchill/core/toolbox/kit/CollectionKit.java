...
import com.smallchill.core.toolbox.support.IEditor;
import com.smallchill.core.toolbox.support.IMatcher;
...
public class CollectionKit {
	...
	public static <T> String join(Iterator<T> iterator, String conjunction) {
		...
		while(iterator.hasNext()){
			...
			if(ArrayKit.isArray(item)) {
				sb.append(ArrayKit.join(ArrayKit.wrap(item), conjunction));
	 ...
	 * @return 过滤后的数组
	 */
	public static <T> Collection<T> filter(Collection<T> collection, IEditor<T> editor) {
	 ...
	 * @return 过滤后的Map
	 */
	public static <K, V> Map<K, V> filter(Map<K, V> map, IEditor<Entry<K, V>> editor) {
	 ...
	 * @return 匹配数量
	 */
	public static <T> int count(Iterable<T> iterable, IMatcher<T> matcher){
		int count = 0;
		for (T t : iterable) {
	...
	public static Map<String, String> zip(String keys, String values, String delimiter) {
		return ArrayKit.zip(StrKit.split(keys, delimiter), StrKit.split(values, delimiter));
	}
