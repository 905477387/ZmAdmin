...
public class Blade {
	...
	public static Blade create(Class<?> modelClass) {
		if (!modelClass.getName().contains(IPackage.packageName)) 
			throw new RuntimeException("no piracy no harm");
			...
			synchronized (Blade.class) {
				...
				if (null == blade) {
					...
					if (null == dbName){
						...
						blade = new Blade(dbName.value(), modelClass);
					}
	...
	public boolean save(Object model) {
		return getSqlManager().insertTemplate(this.modelClass, model, false) > 0;
	}
	...
	public int saveRtId(Object model) {
		return Convert.toInt(saveRtObjId(model), -1);
	}
	 ...
	 * @return
	 */
	public long saveRtLongId(Object model) {
		return Convert.toLong(saveRtObjId(model), -1L);
	}
	...
	public String saveRtStrId(Object model) {
		return Convert.toStr(saveRtObjId(model), "");
	}
	 ...
	 * @return
	 */
	public Object saveRtObjId(Object model) {
		KeyHolder key = new KeyHolder();
		int n = getSqlManager().insertTemplate(this.modelClass, model, key);
		if (n > 0) {
			return key.getKey();
		} else {
			return "";
	...
	public boolean saveAndSetKey(Object model){
		return getSqlManager().insertTemplate(this.modelClass, model, true) > 0;
	}
     ...
     * @param keyName
     */
    public long saveAndSyncKey(Object model, String keyName){
        long id = saveRtLongId(model);
        if (id > 0 && updateBy(keyName + " = #{id}", pk + " = #{id}", CMap.init().set("id", id)))
            return id;
        else
            return 0L;
    }
	...
	*/
	...
	public int deleteByIds(String ids) {
		if(StrKit.isBlank(ids)) return 0;
