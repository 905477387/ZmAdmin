...
public abstract class CurdController<M> extends BaseController {
	...
	public ApiResult save() {
		...
		if (temp) {
			if (null != intercept) {
				...
				ApiResult result = intercept.saveSucceed(ac);
				return result;
			}
	...
	public ApiResult update() {
		...
		if (temp) {
			if (null != intercept) {
				AopContext ac = new AopContext(ctrl, model);
				ApiResult result = intercept.updateSucceed(ac);
				return result;
			}
	...
	public ApiResult remove() {
		...
		if (temp) {
			if (null != intercept) {
				...
				ac.setId(ids);
				ApiResult result = intercept.removeSucceed(ac);
				return result;
			}
	...
	public ApiResult del() {
		...
		if (temp) {
			if (null != intercept) {
				AopContext ac = new AopContext(ctrl);
				ac.setId(ids);
				ApiResult result = intercept.delSucceed(ac);
				return result;
			}
			return success(DEL_SUCCESS_MSG);
		} else {
	...
	public ApiResult restore() {
		...
		if (temp) {
			if (null != intercept) {
				AopContext ac = new AopContext(ctrl);
				ac.setId(ids);
				ApiResult result = intercept.restoreSucceed(ac);
				return result;
			}
